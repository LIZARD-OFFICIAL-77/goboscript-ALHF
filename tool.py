...
if __name__ == '__main__':
    if args.serialize:
        with open(args.i) as i_fp:
            with open(args.o, "w") as o_fp:
                o_fp.write(sep.join(encode(json.load(i_fp))))
    if args.deserialize:
        with open(args.i) as o_fp:
            print(json.dumps(decode(o_fp.read().split(sep)), indent=4))